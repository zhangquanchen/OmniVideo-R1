...
class SoftMultiChoiceAccuracyORM(ORM):
    ...
    def __init__(self, base_url="http://28.49.52.12:22002/v1", judge_model="Qwen3-VL-235B-Instruct", timeout=30):
        """
...
Model's answer: {student_answer}"""
    ...
    def _evaluate_with_llm(self, question: str, student_answer: str, ground_truth: str) -> float:
        ...
        try:
            ...
            # Extract the score from "score: <score>" format
            import re
            # Try to match "score: <number>" pattern (case insensitive)
            match = re.search(r'score\s*:\s*([0-9]*\.?[0-9]+)', result_text, re.IGNORECASE)
            if match:
    ...
    def _extract_question_from_problem(self, problem: str) -> str:
        ...
        Args:
            problem: The full problem text with possible video/audio tags
        ...
        # Remove REASONING_INSTRUCTION suffix if present
        question = question.replace("\nFirst, extract some helpful segments for answering the question in the format: <time>start_time-end_time</time><caption>Key clue description</caption>. Then, think about the reasoning process in the mind and then provide the final answer, i.e., <thinking>Reasoning process here</thinking><answer>Answer here</answer>.", "")
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        """
        rewards = []
        # Get problems/questions from kwargs
        problems = kwargs.get('problem', [])
        
        for i, (content, sol) in enumerate(zip(completions, solution)):
            print("---------------------------------Soft Multi Choice Accuracy Reward Function---------------------------------")
            print(f"Content: {content}")
            print(f"Solution: {sol}")
            
            # Extract question from problem
            question = ""
            if i < len(problems) and problems[i]:
                question = self._extract_question_from_problem(problems[i])
            ...
            try:
                ...
                if content_match:
                    ...
                    reward = self._evaluate_with_llm(question, student_answer, sol.strip())
...
class QueryIntentORM(ORM):
    ...
    def __init__(self, base_url="http://28.49.52.12:22002/v1", judge_model="Qwen3-VL-235B-Instruct", 
        ...
        # System prompt for evaluating caption accuracy
        self.caption_accuracy_system_prompt = """You are an expert evaluator for video content description. Your primary task is to verify the factual correctness of the statements made in the caption against the video content.

Evaluation Criteria:
1. **Fact Verification:** Every visual claim, action, and object mentioned in the caption is explicitly supported by the video footage.
2. **Descriptive Accuracy:** The specific attributes described in the text (such as colors, directions, counts, and sequences) match the actual visual elements.
3. **Absence of Fabrications:** The caption does not contain hallucinations or descriptions of objects and events that are absent from the video.
4. **Logical Audio Consistency:** Audio descriptions appearing in the text are considered accurate as long as they are contextually plausible given the visual scene (e.g., "people talking" is valid if mouths move). They are only penalized if they are blatantly contradictory to the visual context.

Scoring Guidelines:
- 1.0: All statements in the caption are completely accurate and visually verified.
- 0.7-0.9: The caption is mostly accurate, with only very minor phrasing issues or negligible details that deviate slightly from the video.
- 0.4-0.6: The caption describes the correct general topic, but contains specific claims that are false or not present in the video (partial hallucination).
- 0.1-0.3: The caption contains major fabrications, describing actions or objects that clearly do not exist in the video.
- 0.0: The caption describes a completely different scenario or is irrelevant.

You must respond with ONLY a JSON object, no other text or explanation:
{"score": <float between 0.0 and 1.0>, "reason": "<brief explanation for the score>"}"""

        # User prompt template for caption accuracy evaluation
        self.caption_accuracy_user_prompt = """Video segment is provided above.

Caption to evaluate: {caption}

Evaluate whether the descriptions in this caption are accurate based on the video content. Output ONLY the JSON object, no other text:
{{"score": <float>, "reason": "<explanation>"}}"""

        # System prompt for evaluating segment relevance
        self.segment_relevance_system_prompt = """You are an expert evaluator for video-based question answering. Your task is to evaluate the quality of selected video segments based on their ability to support a specific question-answer pair. The input video consists of stitched clips.

Please evaluate the segments according to the following standards:
1. Content Sufficiency & Inference: The segments provide comprehensive visual evidence to support the answer. For questions requiring audio information, the visual context (such as a speaking person, subtitles, specific gestures, or sound-making actions) logically implies and supports the content of the answer.
2. Temporal Precision: The segments are tightly trimmed to include only the key moments, avoiding excessive padding, long periods of inactivity, or redundant scenes.
3. Completeness: The segments capture all necessary steps or details required to derive the solution, ensuring the visual flow is consistent with the answer.

Scoring Guidelines:
- 1.0: The segments contain all necessary visual evidence (or logical visual context for audio cues) and are precisely trimmed with minimal redundancy.
- 0.7-0.9: The segments provide sufficient visual context but include minor irrelevant footage or slightly loose trimming.
- 0.4-0.6: The segments contain the core visual context but suffer from significant redundancy or lack clear visual logical links to the answer.
- 0.1-0.3: The segments provide only partial visual info or the visual context poorly matches the audio-based answer.
- 0.0: The segments are completely irrelevant or fail to visually show the source of the answer.

You must respond with ONLY a JSON object, no other text or explanation:
{"score": <float between 0.0 and 1.0>, "reason": "<brief explanation for the score>"}"""

        # User prompt template for segment relevance evaluation
        self.segment_relevance_user_prompt = """The video segments shown above are the key segments selected by the model.

Question: {question}

Ground Truth Answer: {solution}

Evaluate whether these segments provide sufficient visual evidence (or visual logic for audio questions) to support the answer while remaining concise. Output ONLY the JSON object, no other text:
{{"score": <float>, "reason": "<explanation>"}}"""
    ...
    def _evaluate_caption_accuracy(self, video_path: str, caption: str) -> dict:
        """
        ...
        Returns:
            Dictionary with 'score' (float between 0.0 and 1.0) and 'reason' (str)
        """
        try:
            # print(f"[DEBUG] Starting caption accuracy evaluation for: {video_path}")
            video_base64 = self._encode_video_to_base64(video_path)
            if not video_base64:
                logger.warning("Failed to encode video for caption evaluation")
                return {"score": 0.0, "reason": "Failed to encode video"}
            
            # print(f"[DEBUG] Video encoded, base64 length: {len(video_base64)}")
            messages = [
                {
                    "role": "system",
                    "content": self.caption_accuracy_system_prompt
                },
                {
                    "role": "user",
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": self.caption_accuracy_user_prompt.format(caption=caption)
                        }
                    ]
                }
            ]
            # print(f"[DEBUG] Calling API with model: {self.judge_model}")
            response = self.client.chat.completions.create(
                model=self.judge_model,
                messages=messages,
                max_tokens=256,
            ...
            # print(f"[DEBUG] Caption accuracy response: {response}")
            result_text = response.choices[0].message.content.strip()
            # print(f"[DEBUG] Caption accuracy raw response: {result_text}")
    ...
    def _evaluate_segments_relevance(self, video_path: str, question: str, solution: str) -> dict:
        """
        Use VLM to evaluate if the selected segments are relevant and sufficient for the question-answer pair.
        ...
        """
        try:
            # print(f"[DEBUG] Starting segment relevance evaluation for: {video_path}")
            video_base64 = self._encode_video_to_base64(video_path)
            if not video_base64:
                logger.warning("Failed to encode video for relevance evaluation")
                return {"score": 0.0, "reason": "Failed to encode video"}
            
            # print(f"[DEBUG] Video encoded for relevance, base64 length: {len(video_base64)}")
            messages = [
                {
                    "role": "system",
                    "content": self.segment_relevance_system_prompt
                },
                {
                    "role": "user",
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": self.segment_relevance_user_prompt.format(question=question, solution=solution)
                        }
                    ]
                }
            ]
            
            # print(f"[DEBUG] Calling API for segment relevance with model: {self.judge_model}")
            response = self.client.chat.completions.create(
                model=self.judge_model,
                messages=messages,
                max_tokens=256,
            ...
            # print(f"[DEBUG] Segment relevance response: {response}")
            result_text = response.choices[0].message.content.strip()
            # print(f"[DEBUG] Segment relevance raw response: {result_text}")
            return self._parse_json_response(result_text)
    ...
    def _parse_json_response(self, result_text: str) -> dict:
        """
        Parse JSON response from LLM containing score and reason.
        
        Args:
            result_text: Raw text response from LLM (expected JSON format)
        ...
        default_result = {"score": 0.0, "reason": "Failed to parse response"}
        ...
        try:
            # Match JSON object containing "score"
        ...
        # Fallback: try to extract just a number (backward compatibility)
        try:
            score = float(text)
            return {"score": max(0.0, min(1.0, score)), "reason": "No reason provided"}
        except ValueError:
        ...
        logger.warning(f"Could not parse JSON response from LLM: {result_text}")
        return default_result

    def _parse_score(self, result_text: str) -> float:
        """
        Parse score from LLM response (legacy method for backward compatibility).
    ...
    def __call__(self, completions, solution=None, **kwargs) -> List[float]:
        ...
        Args:
            ...
            solution: Ground truth solutions (list of answers)
        ...
        solutions = solution if isinstance(solution, list) else [solution] * len(completions) if solution else []
        
        for i, content in enumerate(completions):   
            print("---------------------------------Video Segment Caption Reward Function---------------------------------")
            ...
            # Get solution for this sample
            sample_solution = ""
            if i < len(solutions) and solutions[i]:
                sample_solution = solutions[i]
            ...
            try:
                ...
                caption_results = []  # List of dicts with 'score' and 'reason'
                segment_paths = []
                ...
                for (start_time, end_time), caption in pairs:
                    ...
                    if segment_path:
                        ...
                        result = self._evaluate_caption_accuracy(segment_path, caption)
                        ...
                        # print(f"  Caption accuracy - score: {result['score']}, reason: {result['reason']}")
                    else:
                        print(f"  Failed to extract segment, skipping")
                
                # Calculate average caption accuracy (reward1)
                caption_scores = [r["score"] for r in caption_results]
                ...
                reward2 = 0.0
                relevance_result = {"score": 0.0, "reason": "Not evaluated"}
                if segment_paths and question and sample_solution:
                    ...
                    if concat_path:
                        ...
                        # Evaluate segment relevance with question and solution
                        relevance_result = self._evaluate_segments_relevance(concat_path, question, sample_solution)
                        reward2 = relevance_result["score"]
                        # print(f"Reward2 (Segment Relevance) - score: {reward2}, reason: {relevance_result['reason']}")
                    else:
                        relevance_result = {"score": 0.0, "reason": "Failed to concatenate segments"}
                        print(f"Failed to concatenate segments, reward2 = 0.0")
                else:
                    relevance_result = {"score": 0.0, "reason": "No segments, question, or solution available"}
                    print(f"No segments, question, or solution available, reward2 = 0.0")
                
                # ===== Final Reward =====
                final_reward = (reward1 + reward2) / 2.0
                # print(f"QueryIntent Final Reward: {final_reward}")
                
                # Log detailed evaluation results
                evaluation_details = {
                    "caption_accuracy": {
                        "average_score": reward1,
                ...
                print(f"Evaluation Details: {evaluation_details}")
...
class MultiModalModalityAttentionComparisonORM(ORM):
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        for i, (orig_comp, sol) in enumerate(zip(completions, solution)):
            print("---------------------------------ModalityAttention Reward Function---------------------------------")
