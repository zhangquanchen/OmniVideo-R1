...
def load_audio(audio: Union[str, bytes], sampling_rate: int, return_sr: bool = False):
    ...
    try:
        # Try soundfile backend first (faster, but doesn't support MP3)
        ...
        if hasattr(audio_io, 'seek'):
            audio_io.seek(0)
        
        # torchaudio needs a file path or file-like object with proper format detection
        # Write to temp file for reliable format detection
        with tempfile.NamedTemporaryFile(suffix='.mp3', delete=False) as tmp_file:
            tmp_file.write(audio_io.read())
            tmp_path = tmp_file.name
        
        try:
            waveform, sr_native = torchaudio.load(tmp_path)
            # Convert to mono if stereo
            if waveform.shape[0] > 1:
                waveform = waveform.mean(dim=0, keepdim=True)
            # Resample if needed
            if sr_native != sampling_rate:
                resampler = torchaudio.transforms.Resample(sr_native, sampling_rate)
                waveform = resampler(waveform)
            res = (waveform.squeeze(0).numpy(), sampling_rate)
        finally:
            os.unlink(tmp_path)
    
    # end_time = time.time()
    # print(f"Audio loaded in {end_time - start_time} seconds")
