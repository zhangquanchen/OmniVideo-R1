...
class MegatronGRPOTrainer(MegatronRLHFTrainer):
    # Here
    # Instruction to add reasoning process requirement

    ## 业务调试
    # REASONING_INSTRUCTION = "\nPlease first think about the reasoning process in the mind and then provide the user with the answer. The reasoning process and answer are enclosed within <thinking> </thinking> and <answer> </answer> tags, respectively, i.e., <thinking> reasoning process here </thinking><answer> answer here </answer>."
    ...
    # Enable modality_attention comparison mode (generate completions for no-video and no-audio variants)
    ENABLE_MODALITY_ATTENTION_COMPARISON = False
    ...
    def _init_grpo_params(self):
        ...
        # ModalityAttention comparison support
        self.enable_modality_attention_comparison = self.ENABLE_MODALITY_ATTENTION_COMPARISON

    def _create_modality_attention_variant(self, data: Dict, variant_type: str) -> Dict:
        """
        Create a modality_attention variant of the input data by removing video or audio.
        
        Args:
            data: Original input data containing 'messages' field
            variant_type: Either 'no_video' or 'no_audio'
            
        Returns:
            New data dict with video/audio removed from messages
        """
        import re
        
        def remove_video_content(text: str) -> str:
            """Remove <video> tags from text content."""
        ...
        def remove_audio_content(text: str) -> str:
            """Remove <audio> tags from text content."""
            if not isinstance(text, str):
                return text
        ...
        remove_func = remove_video_content if variant_type == 'no_video' else remove_audio_content
            ...
            for msg in new_data['messages']:
                if 'content' in msg:
                    ...
                    if isinstance(content, str):
                        msg['content'] = remove_func(content)
                        ...
                        for item in content:
                            if isinstance(item, dict):
                                ...
                                # Skip video/audio items based on variant type
                                if variant_type == 'no_video' and item_type == 'video':
                                    continue
                                elif variant_type == 'no_audio' and item_type == 'audio':
                                    continue
                                elif item_type == 'text':
                                    new_item = deepcopy(item)
                                    new_item['text'] = remove_func(item.get('text', ''))
                                    new_content.append(new_item)
                                else:
                                    new_content.append(deepcopy(item))
                            else:
                                new_content.append(item)
                        msg['content'] = new_content if new_content else ''
        
        # Process problem field if exists
        if 'problem' in new_data and isinstance(new_data['problem'], str):
            new_data['problem'] = remove_func(new_data['problem'])
        
        # Mark the variant type
        new_data['_modality_attention_variant_type'] = variant_type
        
        return new_data

    def _rollout_modality_attention_variants(self, batch: List[Dict], request_config) -> Dict[str, List]:
        """
        Generate completions for modality_attention variants (no-video, no-audio).
        
        Args:
            batch: Original batch data (already with original completions)
            request_config: Request configuration for inference
            
        Returns:
            Dict with keys 'no_video' and 'no_audio', each containing list of completions
        """
        results = {
            'no_video': [],
            'no_audio': []
        ...
        for variant_type in ['no_video', 'no_audio']:
            # Create modality_attention variants
            modality_attention_batch = []
            for data in batch:
                # Get original messages (before assistant response was added)
                original_messages = deepcopy(data.get('_original_messages', data.get('messages', [])))
                ...
                # Create base modality_attention data with proper fields
                modality_attention_data = {
                    'messages': original_messages,
                    'problem': data.get('problem', ''),
                }
                
                # Copy multimodal fields with appropriate modifications
                if variant_type == 'no_video':
                    # Remove video, keep audio and images
                    if 'images' in data:
                        modality_attention_data['images'] = deepcopy(data['images'])
                    if 'audios' in data:
                        modality_attention_data['audios'] = deepcopy(data['audios'])
                    # Don't copy videos for no_video variant
                elif variant_type == 'no_audio':
                    # Remove audio, keep video and images
                    if 'images' in data:
                        modality_attention_data['images'] = deepcopy(data['images'])
                    if 'videos' in data:
                        modality_attention_data['videos'] = deepcopy(data['videos'])
                    # Don't copy audios for no_audio variant
                
                # Apply text modifications
                modality_attention_data = self._create_modality_attention_variant(modality_attention_data, variant_type)
                modality_attention_batch.append(modality_attention_data)
            
            # Add reasoning instruction to modality_attention variants
            for data in modality_attention_batch:
                messages = data.get('messages', [])
                for message in reversed(messages):
                    if message['role'] == 'user':
                        if isinstance(message['content'], str):
                            message['content'] = message['content'] + self.REASONING_INSTRUCTION
                        elif isinstance(message['content'], list):
                            # Find text content and append instruction
                            found_text = False
                            for item in message['content']:
                                if isinstance(item, dict) and item.get('type') == 'text':
                                    item['text'] = item.get('text', '') + self.REASONING_INSTRUCTION
                                    found_text = True
                                    break
                            # If no text item found, add one
                            if not found_text:
                                message['content'].append({'type': 'text', 'text': self.REASONING_INSTRUCTION})
                        break
            
            # Run rollout for modality_attention variants
            try:
                logger.info(f"Running rollout for {variant_type} modality_attention variants (batch_size={len(modality_attention_batch)})")
                if self.vllm_mode == 'server':
                    modality_attention_outputs = self._server_rollout(modality_attention_batch, request_config)
                elif self.vllm_mode == 'colocate':
                    modality_attention_outputs = self._colocate_rollout(modality_attention_batch, request_config)
                else:
                    modality_attention_outputs = []
                
                # Extract completions
                for output in modality_attention_outputs:
                    if hasattr(output, 'response') and output.response.choices:
                        completion = output.response.choices[0].message.content
                        results[variant_type].append(completion)
                    else:
                        results[variant_type].append("")
                
                logger.info(f"Generated {len(results[variant_type])} {variant_type} completions")
                        
            except Exception as e:
                logger.warning(f"Error generating {variant_type} modality_attention completions: {e}")
                import traceback
                traceback.print_exc()
                results[variant_type] = [""] * len(batch)
    ...
    def _generate_completions(self, batch):
        ...
        # Save original messages before rollout (for modality_attention comparison)
        if self.enable_modality_attention_comparison:
            for data in batch:
                data['_original_messages'] = deepcopy(data.get('messages', []))
        ...
        with context():
            ...
            # Step3: Rollout for original inputs
            outputs: List[RolloutOutput] = self._rollout(batch)
            
            # Step3.5: Generate modality_attention variants if enabled
            modality_attention_completions = None
            if self.enable_modality_attention_comparison:
                logger.info("Generating modality_attention variants (no-video, no-audio)...")
                request_config = self._get_request_config()
                modality_attention_completions = self._rollout_modality_attention_variants(batch, request_config)
                logger.info(f"ModalityAttention completions generated: no_video={len(modality_attention_completions.get('no_video', []))}, "
                           f"no_audio={len(modality_attention_completions.get('no_audio', []))}")
            ...
            # Step5: Add modality_attention completions to batch data for reward computation
            if self.enable_modality_attention_comparison and modality_attention_completions:
                for i, data in enumerate(batch):
                    data['modality_attention_no_video_completion'] = (
                        modality_attention_completions['no_video'][i] 
                        if i < len(modality_attention_completions.get('no_video', [])) else ""
                    )
                    data['modality_attention_no_audio_completion'] = (
                        modality_attention_completions['no_audio'][i]
                        if i < len(modality_attention_completions.get('no_audio', [])) else ""
                    )
    ...
    def _rollout(self, batch) -> List[RolloutOutput]:
        ...
        # Here
        # Add reasoning instruction to user messages and problem field
        for data in batch:
            ...
            # Add instruction to problem field if it exists
            if 'problem' in data:
                data['problem'] = data['problem'] + self.REASONING_INSTRUCTION
