...
EXPERIMENT_SCRIPT="./scripts/src_qi.sh"
...
# ========================================
echo "========================================================"
echo "🔄 Cleaning up python3 processes on all hosts..."
echo "========================================================"
echo ""
...
for i in "${!HOST_IPS[@]}"; do
    ...
    (
        ...
        ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "${CURRENT_IP}" \
            "pkill -9 python3 2>/dev/null; sleep 1; pkill -9 python3 2>/dev/null || true" 2>&1
...
for i in "${!HOST_IPS[@]}"; do
    ...
    PROC_COUNT=$(ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "${CURRENT_IP}" \
        "ps aux | grep python3 | grep -v grep | wc -l" 2>/dev/null || echo "0")
...
if [ "$ALL_CLEAN" = false ]; then
    ...
    for i in "${!HOST_IPS[@]}"; do
        CURRENT_IP=${HOST_IPS[$i]}
        ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "${CURRENT_IP}" \
            "pkill -9 python3 2>/dev/null; sleep 2; pkill -9 python3 2>/dev/null || true" 2>&1 &
    done
    wait
